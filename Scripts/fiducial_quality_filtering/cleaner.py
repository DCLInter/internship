...
The original cleaning.py also had a second code path that read + cleaned
directly from an HDF5 file on disk; that path was never exercised by the
current pipeline (new_checker.py always passed data_ext) and had a real bug
(it left self.demo_info / self.attributes empty when the data_ext path was
used, so csvReport()'s demographic lookups would KeyError). It has been
removed here rather than fixed - see ASSUMPTIONS.txt.
"""
